from gdcdatamodel.models import DataFormat, ExperimentalStrategy, File, Platform
...
def wgs(graph, source):
    ...
    illumina_plus_hiseq_x_ten = Platform.name.astext.contains("Illumina") | (
        Platform.name.astext == "HiSeq X Ten"
    )
    bam = DataFormat.name.astext == "BAM"
    all_file_ids_sq = (
        graph.nodes(File.node_id)
