...
class ESMapper:
    ...
    def get_case_tree():
        ...
        case_tree.sample.portion.analyte.aliquot.annotation.corr = (
            ONE_TO_MANY,
            "annotations",
        )
    ...
    def get_descriptions_from_tree(cls, tree, root_name, path=""):
        ...
        for label in [key for key in tree if key != "corr"]:
            ...
            # recur
            descriptions.update(
                cls.get_descriptions_from_tree(
    ...
    def get_descriptions(cls):
        ...
        descriptions.update(
            cls.get_descriptions_from_tree(cls.get_case_tree(), "cases")
        )
        descriptions.update(
            cls.get_descriptions_from_tree(cls.get_file_tree(), "files")
        )
        descriptions.update(
            cls.get_descriptions_from_tree(cls.get_project_tree(), "projects")
        )
    ...
    def get_properties_by_category(cls, category):
        doc = Dict()
        classes = (
            c for c in Node.get_subclasses() if c._dictionary["category"] == category
        )
    ...
    def get_file_es_mapping(cls, include_case=True, is_root=True):
        ...
        files.properties = Dict(
            {
                ...
                for key, value in cls.get_base_properties(
                    node.label, include_id=False
                ).items()
            }
        )

        files.properties = cls._walk_tree(cls.get_file_tree(), files.properties)
        if not include_case:
        ...
        files.properties.associated_entities.properties.update(
            cls.multifield("case_id")
        )
        ...
        if include_case:
            files.properties.cases = cls.get_case_es_mapping(
                include_file=False, is_root=False
            )
    ...
    def get_case_es_mapping(cls, include_file=True, is_root=True):
        ...
        if include_file:
            case.properties.files = cls.get_file_es_mapping(
                include_case=False, is_root=False
            )
    ...
    def get_annotation_es_mapping(cls, include_file=True):
        ...
        annotation.properties.update(
            Dict({"project": {"properties": cls.get_base_properties("project")}})
        )
    ...
    def get_project_es_mapping(cls):
        ...
        project.properties = cls._walk_tree(
            cls.get_project_tree(), cls.get_base_properties("project")
        )
    ...
    def add_file_autocomplete(files):
        ...
        files.properties.file_autocomplete.fields.analyzed.analyzer = (
            "autocomplete_analyzed"
        )
        files.properties.file_autocomplete.fields.analyzed.search_analyzer = (
            "lowercase_keyword"
        )
        files.properties.file_autocomplete.fields.analyzed.type = "text"
        files.properties.file_autocomplete.fields.lowercase.analyzer = (
            "lowercase_keyword"
        )
        files.properties.file_autocomplete.fields.lowercase.type = "text"
        files.properties.file_autocomplete.fields.prefix.analyzer = (
            "autocomplete_prefix"
        )
        files.properties.file_autocomplete.fields.prefix.search_analyzer = (
            "lowercase_keyword"
        )
    ...
    def add_case_autocomplete(case):
        ...
        case.properties.case_autocomplete.fields.analyzed.search_analyzer = (
            "lowercase_keyword"
        )
        case.properties.case_autocomplete.fields.analyzed.type = "text"
        case.properties.case_autocomplete.fields.lowercase.analyzer = (
            "lowercase_keyword"
        )
        ...
        case.properties.case_autocomplete.fields.prefix.search_analyzer = (
            "lowercase_keyword"
        )
        ...
        case.properties.project.properties.intended_release_date.copy_to = [
            "case_autocomplete"
        ]
    ...
    def add_project_autocomplete(project):
        ...
        project.properties.project_autocomplete.fields.analyzed.search_analyzer = (
            "lowercase_keyword"
        )
        project.properties.project_autocomplete.fields.analyzed.type = "text"
        project.properties.project_autocomplete.fields.lowercase.analyzer = (
            "lowercase_keyword"
        )
        project.properties.project_autocomplete.fields.lowercase.type = "text"
        project.properties.project_autocomplete.fields.prefix.analyzer = (
            "autocomplete_prefix"
        )
        project.properties.project_autocomplete.fields.prefix.search_analyzer = (
            "lowercase_keyword"
        )
    ...
    def add_annotation_autocomplete(annotation):
        ...
        annotation.properties.annotation_autocomplete.fields.prefix.analyzer = (
            "autocomplete_prefix"
        )
        annotation.properties.annotation_autocomplete.fields.prefix.search_analyzer = (
            "lowercase_keyword"
        )
